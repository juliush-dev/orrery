  ...
  for (const size of [{width:1440,height:900}, {width:1180,height:760}, {width:400,height:780}]) {
    for (const reducedMotion of ['no-preference', 'reduce']) {
      ...
      await page.setContent(`<!doctype html><style>
        ...
        #st-path{position:absolute;top:12px;left:12px;font:18px sans-serif}
      ...
      </g></g></svg><div class="tools"><button id="fit">Fit</button></div></div>
      <span id="st-path"></span><script>${motion}\n${stage}\n
      ...
      await page.locator('#st-path button').first().click();
      await page.clock.runFor(600);
